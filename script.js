// v12.4.4 – FULL RESTORE + stable share typing + budget OFF + header period
// Storage keys
...
// Defaults: Global budget OFF at first run; per-row OFF
...
// Save/Load
let saveTimer=null; const scheduleSave=()=>{ clearTimeout(saveTimer); saveMonth(); saveCtx(); };
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  try{ const ctx=JSON.parse(localStorage.getItem(CTX_KEY)||'null'); const yearSel=document.getElementById('year'); if(ctx && [...yearSel.options].some(o=>o.value==ctx.y)){ yearSel.value=ctx.y; currentStore=ctx.store||'neutral'; loadMonth(ctx.m); return; } }catch{}
  const m=String(new Date().getMonth()+1).padStart(2,'0'); loadMonth(m);
});

// Service worker
if('serviceWorker' in navigator){ window.addEventListener('load',()=>{ navigator.serviceWorker.register('sw.js?v=12.4.4'); }); }
