// v11.2.2 LTS (stabil) – robust lagring & månader, global 🎯 ON, per‑rad OFF
// --- Storage namespaces ---
const CTX_KEY='budget_ctx';
...
// --- Defaults ---
function ensureDefaults(){ const p=getPrefs(); if(p.budgetMode===undefined){ // LTS-önskemål: Global ON
    p.budgetMode=true; setPrefs(p);
  }
  const b=getBudget(); b.rows=b.rows||{}; b.rowsMode=b.rowsMode||{}; // LTS-önskemål: per-rad OFF
...
// --- Month storage: choose best source (don’t migrate implicitly) ---
function candidateKeys(y,m){ return [ ['neutral',keyNeutral(y,m)], ['v113',key113(y,m)], ['v112',key112(y,m)] ] }
function readBestMonth(y,m){ let best=null; candidateKeys(y,m).forEach(([mode,k])=>{ const raw=localStorage.getItem(k); if(!raw) return; try{ const d=JSON.parse(raw); const richness = JSON.stringify(d).length + Object.keys(d||{}).length*10; if(!best || richness>(best.richness||0)){ best={mode,key:k,data:d,richness} } }catch{} }); return best }
function saveMonthPackage(y,m, data){ let key;
  if(currentStore==='neutral') key=keyNeutral(y,m);
  else if(currentStore==='v113') key=key113(y,m);
  else if(currentStore==='v112') key=key112(y,m);
  else { // auto -> use existing if any, else neutral
    const found=readBestMonth(y,m); key = found? found.key : keyNeutral(y,m); currentStore = found? found.mode : 'neutral';
  }
  localStorage.setItem(key, JSON.stringify(data));
}

function scanYear(y){ const months=['01','02','03','04','05','06','07','08','09','10','11','12']; const res=[]; months.forEach(m=>{ const best=readBestMonth(y,m); if(best) res.push({m, mode:best.mode, size:best.richness}); }); return res }
...
function renderPersons(list){ const grid=document.getElementById('personGrid'); grid.innerHTML=''; (list||defaultPersons()).forEach((p,i)=> addPersonCard(p.name,p.salary, i===0)); updateSharesUI(true); }
function addPersonCard(name='Du', salary='', isPrimary=false){ const grid=document.getElementById('personGrid'); const card=document.createElement('div'); card.className='person-card'; card.innerHTML=`<div class="head"><span class="ico">${isPrimary?'9d1':'464'}</span><input class="name" type="text" placeholder="Namn" value="${name}"></div><input class="salary" type="text" inputmode="decimal" placeholder="Lön" value="${salary}"><button class="del" ${isPrimary?'disabled':''} title="Ta bort">5d13fb</button>`; const nameEl=card.querySelector('.name'); const salEl=card.querySelector('.salary'); nameEl.addEventListener('input',()=>{ calc(); scheduleSave(); updateSharesUI(true); }); salEl.addEventListener('input',()=>{ calc(); scheduleSave(); updateSharesUI(true); }); card.querySelector('.del').addEventListener('click',()=>{ if(!isPrimary){ card.remove(); calc(); scheduleSave(); updateSharesUI(true); }}); grid.appendChild(card); }
...
function applyBudgetVisibility(){ const p=getPrefs(); const on=!!p.budgetMode; const hdrBtn=document.getElementById('budgetModeToggle'); if(hdrBtn) hdrBtn.textContent=`3af Budgetläge: ${on?'ON':'OFF'}`; document.querySelectorAll('label.cat, label.custom').forEach(applyBudgetVisibilityFor); const auditSec=document.querySelector('.sec.audit'); if(auditSec) auditSec.style.display = on? 'block':'none'; }
...
document.addEventListener('click', (e)=>{
  const budgetBtn = e.target.closest('.budgetCat'); if(budgetBtn){ const label = budgetBtn.closest('label.cat, label.custom'); if(label) toggleRowBudget(label); }
...
function updateSharesUI(forceRebuild=false){
  ...
  const paidTotals=getPaidTotals(); [...document.querySelectorAll('.person-share')].forEach(div=>{ const name=div.dataset.name; const person=persons.find(x=> x.name===name ); const andelPct = total>0? ( (person?.salary||0)/total*100 ):0; const skall = sharedTotal * ( total>0? ((person?.salary||0)/total): 0 ); const paid=paidTotals[name]||0; const progress = skall>0? Math.round(paid*100/skall): 0; const saldo = paid - skall; div.querySelector('.skall').textContent=kr(skall); div.querySelector('.paid').textContent=kr(paid); div.querySelector('.progress').textContent=(progress)+'%'; div.querySelector('.saldo').textContent=kr(saldo); const bar=div.querySelector('.bar>div'); bar.style.width=(Math.min(100,Math.max(0,progress)))+'%'; });
}

// Payments per row (identisk mot tidigare full)
function togglePaymentsDrawer(label){ let d=label.querySelector('.drawer'); if(d){ d.remove(); return } d=document.createElement('div'); d.className='drawer'; const id=label.dataset.id; const amount = label.classList.contains('custom')? parseDec(label.querySelector('.c-val')?.value||''): parseDec(document.getElementById(id)?.value||''); const persons=readPersons(); const paidData=getRowPaid()[id]||{}; const mustSum=(getRowPaid()[id]?.__mustSum)||false; const form=document.createElement('div'); form.innerHTML=`<div class="hdr">Radbelopp: <strong>${kr(amount)}</strong> | 🔗 Delas: ${label.classList.contains('share')? 'JA':'NEJ'}</div>`;
...
function renderAudit(){ const wrap=document.getElementById('auditTable'); if(!wrap) return; const prefs=getPrefs(); wrap.innerHTML=''; if(!prefs.budgetMode){ wrap.innerHTML='<p>Budgetläge är AV. Tänd Budgetläge för att visa avstämning.</p>'; return } const snap=getSnapshot(); const b=getBudget(); const budgetRows=(b.rows)||{}; const rowsMode=(b.rowsMode)||{}; if(!snap){ wrap.innerHTML='<p>Ingen snapshot ännu. Stäng månaden för att låsa utfallet.</p>'; return } const t=snap.totals;
  ...
  const bFixed=sumBudgetOfSection('fixed', budgetRows, rowsMode); const bVar=sumBudgetOfSection('var', budgetRows, rowsMode); const bSave=sumBudgetOfSection('save', budgetRows, rowsMode); const bTotal=(bFixed+bVar+bSave); addRow('Fasta', bFixed, t.fixed); addRow('Rörliga', bVar, t.var); addRow('Spar/Skuld', bSave, t.spar); addRow('TOTAL', bTotal, t.fixed+t.var+t.spar);
  table.appendChild(thead); table.appendChild(tbody); wrap.appendChild(table);
  const table2=document.createElement('table'); const thead2=document.createElement('thead'); const tbody2=document.createElement('tbody'); thead2.innerHTML='<tr><th>Rad</th><th>Budget</th><th>Utfallet</th><th>Avvikelse</th><th>Avvikelse %</th></tr>'; Object.keys(budgetRows).forEach(id=>{ if(rowsMode[id]===false) return; const bval=budgetRows[id]; const actual=t.rows[id]||0; const diff=actual-(bval||0); const pct=bval>0? ((actual/bval-1)*100): null; const tr=document.createElement('tr'); const name=prettyNameForId(id); tr.innerHTML=`<td>${name}</td><td>${kr(bval||0)}</td><td>${kr(actual)}</td><td>${diff>=0?'+':'−'}${kr(Math.abs(diff))}</td><td>${pct==null?'—': pct.toFixed(1)+'%'}</td>`; tbody2.appendChild(tr) }); table2.appendChild(thead2); table2.appendChild(tbody2); wrap.appendChild(document.createElement('br')); wrap.appendChild(table2); }

function sumBudgetOfSection(section, budgetRows, rowsMode){ const base = section==='fixed' ? ['boende','bredband','telefoni','barnomsorg1','barnomsorg2','fackforbund','personfors','barnfors','djurfors','hemfors','bilfors','fors_ovrigt','stream_netflix','stream_spotify','stream_hbo','stream_ovrigt1','stream_ovrigt2'] : section==='var' ? ['mat','hushall','klader','nojen','halsa','energi','ror_ovrigt','bransle','service','skatt','parkering','fordon_ovrigt'] : ['spar1','spar2','spar3','spar4','spar5','spar6','skuld']; let s=0; base.forEach(id=>{ if(rowsMode[id]===false) return; if(budgetRows[id]) s+= parseDec(budgetRows[id]) }); document.querySelectorAll('label.custom').forEach(l=>{ const id=l.dataset.id; const sec=l.dataset.section; if(rowsMode[id]===false) return; if(sec===section && budgetRows[id]) s+= parseDec(budgetRows[id]) }); return s }
...
// Year summary (reads via best source)
const mkey=m=>`auto`;
...
async function exportPDF(){ const { jsPDF } = window.jspdf; const doc = new jsPDF({orientation:'p', unit:'pt'}); const m=40; let y=m; const T=(t,v='')=>{ doc.text(`${t} ${v}`, m, y); y+=18; };
  ...
  const sharedTotal=document.getElementById('sharedTotal')?.textContent||''; const sharePct=document.getElementById('sharePct')?.textContent||''; if(sharedTotal) doc.text(`Delad summa: ${sharedTotal} | Procent av lön: ${sharePct}`, m, y);
  doc.save(`budget_${Y()}-${currentMonth}.pdf`);
}

// Save / Load (uses selected storage)
...
function readCustom(section){ const gridId=section==='fixed'?'fixedBaseGrid':section==='var'?'varBaseGrid':'saveBaseGrid'; return [...document.querySelectorAll(`#${gridId} label.custom`)].map(l=>({ name:l.querySelector('.c-name')?.value||'', value: parseDec(l.querySelector('.c-val')?.value||''), share: l.classList.contains('share'), id:l.dataset.id })) }
...
function loadMonth(m){ currentMonth=m; document.querySelectorAll('.month-buttons button').forEach(b=>b.classList.toggle('active',b.dataset.month===m));
  ...
  // choose best source
  const best=readBestMonth(Y(), m);
  if(best){ const d=best.data; currentStore=best.mode; ids.forEach(id=>{ const el=document.getElementById(id); if(el && d[id]!==undefined) el.value=d[id] }); renderPersons(d['_persons']&&d['_persons'].length? d['_persons']: defaultPersons()); (d['_fixedCustom']||[]).forEach(c=>{ const l=makeCustomLabel('fixed', c.name, c.value, !!c.share); document.getElementById('fixedBaseGrid').appendChild(l); }); (d['_varCustom']||[]).forEach(c=>{ const l=makeCustomLabel('var', c.name, c.value, !!c.share); document.getElementById('varBaseGrid').appendChild(l); }); (d['_saveCustom']||[]).forEach(c=>{ const l=makeCustomLabel('save', c.name, c.value, !!c.share); document.getElementById('saveBaseGrid').appendChild(l); }); if(d['_hidden']) setHidden(d['_hidden']); if(d['_shared']) setShared(d['_shared']); if(d['_viewPrefs']) setViewPrefs(d['_viewPrefs']); if(d['_rowPaid']) setRowPaid(d['_rowPaid']); if(d['_budget']) setBudget(d['_budget']); if(d['_snapshot']) setSnapshot(d['_snapshot']); if(d['_prefs']) setPrefs(d['_prefs']); toast(`Data lästes från ${best.mode.toUpperCase()}`);
  } else {
    currentStore='neutral'; renderPersons(defaultPersons());
  }
...
function makeCustomLabel(section, name='', value='', share=false){ const label=document.createElement('label'); label.className='cat custom'; const id=`c_${section}_${++customIdSeq}`; label.dataset.id=id; label.dataset.section=section; if(share) label.classList.add('share'); const ico='🧩';
  ...
  <div class="row-ctl"><button class="hideCat" title="Dölj">👁️</button><button class="shareCat" title="Delas">🔗</button><button class="payCat" title="Betalningar">💳</button><button class="budgetCat" title="Budget ON/OFF">🎯</button></div>
...
// Theme & header
function applyTheme(){ const t=localStorage.getItem('budget_theme')||'light'; if(t==='dark') document.documentElement.setAttribute('data-theme','dark'); else document.documentElement.removeAttribute('data-theme') }
...
// Scan & recover UI
function scanAndReport(){ const y=Y(); const found=scanYear(y); if(!found.length){ toast('Inget att återställa för valt år'); return } const lines=found.map(x=> `${x.m}: ${x.mode.toUpperCase()} (${x.size} bytes)`); alert(`Hittade data:\n`+lines.join('\n')+"\n\nÖppna en månad för att läsa in den automatiskt."); }
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const scanBtn=document.getElementById('scanDataBtn'); if(scanBtn) scanBtn.addEventListener('click', scanAndReport);

  try{ const ctx=JSON.parse(localStorage.getItem(CTX_KEY)||'null'); if(ctx && [...document.getElementById('year').options].some(o=>o.value==ctx.y)){ document.getElementById('year').value=ctx.y; currentStore=ctx.store||'auto'; loadMonth(ctx.m); return; } }catch{}
  const m=String(new Date().getMonth()+1).padStart(2,'0'); loadMonth(m);
});

// Service worker – version bump to bust old caches
if('serviceWorker' in navigator){ window.addEventListener('load',()=>{ navigator.serviceWorker.register('sw.js?v=1122LTS'); }); }
