// v12 – emojis/symboler fix, nattläge fixat, egna rader per sektion, focus‑clear på namn/lön
// Storage keys (neutral + back-compat)
const CTX_KEY='budget_ctx';
const PREFS_KEY='budget_prefs';
const BUDGET_KEY='budget_rows';
...
function ensureDefaults(){ const p=getPrefs(); if(p.budgetMode===undefined){ p.budgetMode=true; setPrefs(p); } const b=getBudget(); b.rows=b.rows||{}; b.rowsMode=b.rowsMode||{}; document.querySelectorAll('label.cat, label.custom').forEach(l=>{ const id=l.dataset.id; if(!(id in b.rowsMode)) b.rowsMode[id]=false; }); setBudget(b); }
...
// People (focus-clear on name/salary)
...
function focusClearOnBlur(e){ const el=e.target; /* keep empty if user leaves it blank */ }
...
function calc(){
  ...
  const fixed = sumSection(['boende','bredband','telefoni','fackforbund']) + fors + stream + sumCustom('fixed'); setText('sum_fixed', kr(fixed));
  const vari = sumSection(['mat','hushall','klader','energi']) + sumCustom('var'); setText('sum_var', kr(vari));
  const spar = sumSection(['spar1','spar2','skuld']) + sumCustom('save'); setText('sum_spar', kr(spar));
...
// Year
function getMonthTotals(y,m){ const found=readBestMonth(y,m); if(!found) return {income:0,fixed:0,var:0,spar:0,res:0}; try{ const d=found.data; const dec=x=>parseDec(d[x]); const hidden=d['_hidden']||[]; const H=id=> hidden.includes(id)?0:dec(id); const persons=(d['_persons']||[{name:'',salary:0}]).reduce((s,p)=> s+(parseDec(p.salary)||0),0); const income = persons + dec('formaner') + dec('barnbidrag') + dec('ink_ovrigt'); const fixed = H('boende')+H('bredband')+H('telefoni')+H('fackforbund') + (H('personfors')+H('barnfors')+H('djurfors')+H('hemfors')+H('bilfors')+H('fors_ovrigt')) + (H('stream_netflix')+H('stream_spotify')+H('stream_hbo')+H('stream_ovrigt1')+H('stream_ovrigt2')) + (d['_fixedCustom']||[]).reduce((s,c)=> s+(parseDec(c.value)||0),0); const vari = H('mat')+H('hushall')+H('klader')+H('energi') + (d['_varCustom']||[]).reduce((s,c)=> s+(parseDec(c.value)||0),0); const spar = H('spar1')+H('spar2')+H('skuld') + (d['_saveCustom']||[]).reduce((s,c)=> s+(parseDec(c.value)||0),0); const res = income - fixed - vari - spar; return {income,fixed,var:vari,spar,res}; }catch{ return {income:0,fixed:0,var:0,spar:0,res:0} } }
...
function exportMonthCSV(){ const rows=[["År",Y()],["Månad",currentMonth]]; const add=(a,b)=>rows.push([a,b]); const read=id=>document.getElementById(id)?.value||''; rows.push([]);
  ...
  ['boende','bredband','telefoni','fackforbund','personfors','barnfors','djurfors','hemfors','bilfors','fors_ovrigt','stream_netflix','stream_spotify','stream_hbo','stream_ovrigt1','stream_ovrigt2','mat','hushall','klader','energi','spar1','spar2','skuld'].forEach(id=> add(id, read(id)) ); rows.push([]);
...
function loadMonth(m){ currentMonth=m; document.querySelectorAll('.month-buttons button').forEach(b=>b.classList.toggle('active',b.dataset.month===m));
  const ids=['period','namn','formaner','barnbidrag','ink_ovrigt','boende','bredband','telefoni','fackforbund','personfors','barnfors','djurfors','hemfors','bilfors','fors_ovrigt','stream_netflix','stream_spotify','stream_hbo','stream_ovrigt1','stream_ovrigt2','mat','hushall','klader','energi','spar1','spar2','skuld']; ids.forEach(id=>{ const el=document.getElementById(id); if(el) el.value='' }); ['fixedBaseGrid','varBaseGrid','saveBaseGrid'].forEach(g=>{ document.querySelectorAll(`#${g} label.custom`).forEach(n=>n.remove()); });
  const found=readBestMonth(Y(), m);
  if(found){ try{ const d=found.data; ids.forEach(id=>{ const el=document.getElementById(id); if(el && d[id]!==undefined) el.value=d[id] }); renderPersons(d['_persons']&&d['_persons'].length? d['_persons']: defaultPersons()); (d['_fixedCustom']||[]).forEach(c=>{ const l=makeCustomLabel('fixed', c.name, c.value, !!c.share); document.getElementById('fixedBaseGrid').appendChild(l); }); (d['_varCustom']||[]).forEach(c=>{ const l=makeCustomLabel('var', c.name, c.value, !!c.share); document.getElementById('varBaseGrid').appendChild(l); }); (d['_saveCustom']||[]).forEach(c=>{ const l=makeCustomLabel('save', c.name, c.value, !!c.share); document.getElementById('saveBaseGrid').appendChild(l); }); if(d['_hidden']) setHidden(d['_hidden']); if(d['_shared']) setShared(d['_shared']); if(d['_viewPrefs']) setViewPrefs(d['_viewPrefs']); if(d['_rowPaid']) setRowPaid(d['_rowPaid']); if(d['_budget']) setBudget(d['_budget']); if(d['_snapshot']) setSnapshot(d['_snapshot']); if(d['_prefs']) setPrefs(d['_prefs']); toast(`Data lästes`);
    }catch{}
  } else {
...
// Theme (dark mode robust)
function applyTheme(){ const t=localStorage.getItem('budget_theme')||'light'; if(t==='dark'){ document.documentElement.setAttribute('data-theme','dark'); const d=$('#darkToggle'); if(d) d.textContent='☀️'; } else { document.documentElement.removeAttribute('data-theme'); const d=$('#darkToggle'); if(d) d.textContent='🌙'; } }
